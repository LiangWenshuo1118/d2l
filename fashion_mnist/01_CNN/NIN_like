...
def NiN_block(in_channels, out_channels, kernel_size,strides, padding):
    return nn.Sequential(
            nn.Conv2d(in_channels, out_channels, kernel_size, strides, padding),  # 卷积核大小为3x3
            nn.ReLU(),  # 使用ReLU激活函数
            nn.Conv2d(out_channels, out_channels, kernel_size=1, strides, padding),  # 1*1卷积
            nn.ReLU(),  # 使用ReLU激活函数
            nn.Conv2d(out_channels, out_channels, kernel_size=1, strides, padding),  # 1*1卷积
            nn.ReLU(),  # 使用ReLU激活函数
...
# 定义一个类似VGG的神经网络模型
class NiN_like(nn.Module):
    def __init__(self):
        super(NiN_like, self).__init__()  # 调用父类的构造函数
        # 特征提取部分，包含多层卷积和池化层
        self.features = nn.Sequential(
            NiN_block(in_channels=1, out_channels=32, kernel_size=3, strides=1, padding=1),
            NiN_block(in_channels=32,out_channels=10, kernel_size=3, strides=1, padding=1),           
